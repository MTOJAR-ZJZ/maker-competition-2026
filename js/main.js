...
(function initCountdown() {
  // 报名截止日期：2026年7月30日，按当天 23:59:59 计算
  const deadline = new Date('2026-07-30T23:59:59+08:00');

  function getMonthDiff(start, end) {
    let months = (end.getFullYear() - start.getFullYear()) * 12 + (end.getMonth() - start.getMonth());
    const testDate = new Date(start);
    testDate.setMonth(start.getMonth() + months);
  ...
  function update() {
    ...
    if (diff <= 0) {
      ...
      ['nav-months', 'nav-days', 'nav-hours', 'nav-minutes'].forEach((id) => {
    ...
    const months = getMonthDiff(now, deadline);
    const afterMonths = new Date(now);
    afterMonths.setMonth(now.getMonth() + months);

    diff = deadline - afterMonths;
    const days = Math.floor(diff / (1000 * 60 * 60 * 24));
    const hours = Math.floor((diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    const minutes = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));

    document.getElementById('days').textContent = String(months).padStart(2, '0');
    document.getElementById('hours').textContent = String(days).padStart(2, '0');
    document.getElementById('minutes').textContent = String(hours).padStart(2, '0');
    ...
    if (navMonths && navDays && navHours && navMinutes) {
      navMonths.textContent = String(months).padStart(2, '0');
      navDays.textContent = String(days).padStart(2, '0');
      navHours.textContent = String(hours).padStart(2, '0');
